...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($searchQuery)) {
        $query .= " AND (a.Serial_Number LIKE ? OR a.Engraving LIKE ? OR u.First_Name LIKE ? OR u.Last_Name LIKE ? OR aa.location LIKE ?)";
        $searchParam = "%$searchQuery%";
        $params = array_fill(0, 5, $searchParam); // Fill the params array with the same search parameter
        $types = str_repeat('s', 5); // Repeat 's' for each parameter type
        }
    ...
    <form method="POST" action="search.php">
        <div class="form-row">
            <div class="form-group col-md-8">
                <label for="search_query">Search (Serial Number, Engraving, User, Location)</label>
